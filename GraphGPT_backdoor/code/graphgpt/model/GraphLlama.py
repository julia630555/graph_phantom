...
def infer_graph_hidden_size_from_tower(graph_tower):
    if graph_tower is None:
        return None

    args = getattr(graph_tower, "args", None)
    if args is not None:
        for key in ("gnn_output", "embed_dim", "att_d_model", "gnn_hid", "gnn_input"):
            if hasattr(args, key):
                val = _try_int(getattr(args, key))
                if val is not None and val > 0:
                    return val

    if hasattr(graph_tower, "inverW_P") and hasattr(graph_tower.inverW_P, "out_features"):
        val = _try_int(graph_tower.inverW_P.out_features)
        if val is not None and val > 0:
            return val

    vars_attr = getattr(graph_tower, "vars", None)
    if vars_attr is not None:
        try:
            if len(vars_attr) >= 3 and hasattr(vars_attr[2], "shape"):
                val = _try_int(vars_attr[2].shape[0])
                if val is not None and val > 0:
                    return val
        except Exception:
            pass

    cfg = getattr(graph_tower, "config", None)
    if cfg is not None:
        for key in ("graph_hidden_size", "hidden_size", "dim"):
            if hasattr(cfg, key):
                val = _try_int(getattr(cfg, key))
                if val is not None and val > 0:
                    return val
...
def ensure_graph_hidden_size(config, graph_tower=None, fallback=128):
    current = _try_int(getattr(config, "graph_hidden_size", None))
    if current is not None and current > 0:
        return current

    inferred = infer_graph_hidden_size_from_tower(graph_tower)
    if inferred is None:
        inferred = fallback

    config.graph_hidden_size = inferred
    return inferred
...
class GraphLlamaModel(LlamaModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        if hasattr(config, "use_graph_proj"):
            ensure_graph_hidden_size(config, getattr(self, "graph_tower", None))
    ...
    def initialize_graph_modules(self, graph_tower, graph_select_layer,
        ...
        if not hasattr(self, 'graph_projector'):
            ensure_graph_hidden_size(self.config, graph_tower)
    ...
    def forward(
        ...
        use_backdoor_trigger: Optional[bool] = False,
        poison_mask: Optional[torch.Tensor] = None,
        trigger_temperature: Optional[float] = 1.0,
        trigger_forward_hard: Optional[bool] = True,
        trigger_use_ste: Optional[bool] = True,
        ...
        if graph_tower is not None and (input_ids.shape[1] != 1 or self.training) and graph_data is not None:
            ...
            graph_poison_flags = []
            graph_tower_trainable = self.training and any(
                p.requires_grad for p in graph_tower.parameters()
            )
            graph_tower_context = nullcontext() if graph_tower_trainable else torch.no_grad()
            with graph_tower_context:
                if type(graph_data) is list:
                    ...
                    if type(graph_data[0]) is Data:
                        for idx, g in enumerate(graph_data):
                            ...
                            is_poison = False
                            if poison_mask is not None and idx < poison_mask.shape[0]:
                                is_poison = bool(poison_mask[idx].item())
                            graph_poison_flags.append(is_poison)
                    elif type(graph_data[0]) is dict:
                        for idx, g_dict in enumerate(graph_data):
                            ...
                            if poison_mask is not None and idx < poison_mask.shape[0]:
                                is_poison = bool(poison_mask[idx].item())
                            graph_poison_flags.extend([is_poison, is_poison])
                else:
                    raise ValueError(f'graph_node_reps is expected to be a list but got {type(graph_data)}')
            if type(graph_data) is list:
                if use_backdoor_trigger:
                    trigger_module = getattr(self, "backdoor_trigger", None)
                    if trigger_module is None:
                        raise ValueError("use_backdoor_trigger=True but model.backdoor_trigger is not initialized.")

                    used_trigger = False
                    updated_features = []
                    for feature_idx, node_feature in enumerate(graph_node_features):
                        is_poison = graph_poison_flags[feature_idx] if feature_idx < len(graph_poison_flags) else False
                        if is_poison:
                            node_feature = trigger_module.apply_to_node_features(
                                node_feature,
                                temperature=float(trigger_temperature if trigger_temperature is not None else 1.0),
                                forward_hard=bool(trigger_forward_hard),
                                use_ste=bool(trigger_use_ste),
                            )
                            used_trigger = True
                        updated_features.append(node_feature)
                    if not used_trigger and len(updated_features) > 0:
                        zero_link = trigger_module.zero_connection(
                            temperature=float(trigger_temperature if trigger_temperature is not None else 1.0),
                            forward_hard=bool(trigger_forward_hard),
                            use_ste=bool(trigger_use_ste),
                        ).to(device=updated_features[0].device, dtype=updated_features[0].dtype)
                        updated_features[0] = updated_features[0] + zero_link
                    graph_node_features = updated_features
                ...
                raise ValueError(f'graph_node_reps is expected to be a list but got {type(graph_data)}')
            dummy_graph_features = torch.zeros(256, self.config.graph_hidden_size, device=inputs_embeds.device, dtype=inputs_embeds.dtype)
...
class GraphLlamaForCausalLM(LlamaForCausalLM):
    ...
    def forward(
        ...
        outputs = self.model(
            ...
            use_backdoor_trigger=use_backdoor_trigger,
            poison_mask=poison_mask,
            trigger_temperature=trigger_temperature,
            trigger_forward_hard=trigger_forward_hard,
            trigger_use_ste=trigger_use_ste,
        )
        ...
        model_inputs.update(
            {
                ...
                "use_backdoor_trigger": kwargs.get("use_backdoor_trigger", False),
                "poison_mask": kwargs.get("poison_mask", None),
                "trigger_temperature": kwargs.get("trigger_temperature", 1.0),
                "trigger_forward_hard": kwargs.get("trigger_forward_hard", True),
                "trigger_use_ste": kwargs.get("trigger_use_ste", True),
